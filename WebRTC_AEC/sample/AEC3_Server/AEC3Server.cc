...
static FILE *s_pB4AECPathFile = NULL;
static FILE *s_pAFAECPathFile = NULL;
...
)
{
	...
	if(pu8TempRecBuf == NULL){
		ERRPRINT("Server - Cannot alloc temp record buffer \n");
		return 0;
	}
	...
	while(s_bThreadRun){
		...
		if(eErr == ERR_ALSA_NONE){
			...
			if (s_bEnEchoCancel){
				...
				if(s_bStartRecToFile && s_pB4AECPathFile == NULL){
					//Create before AEC Path file
					s_pB4AECPathFile = fopen ( "/mnt/mmc/AEC3_B4_AEC_path", "w");

					if(s_pB4AECPathFile == NULL){
						printf("Unable create befor AEC path file \n");
						s_bStartRecToFile = false;
					}
				}
				
				if(s_pB4AECPathFile){
					fwrite(s_sAudioRecCtx.pDataVAddr, 1, s_u32RecFrameSize, s_pB4AECPathFile);
				}

				if(s_bStartRecToFile == false && s_pB4AECPathFile){
					//close play Path file
					fclose(s_pB4AECPathFile);			
					s_pB4AECPathFile = NULL;
				}
...
//				DEBUGPRINT("echo cancel spend time is %d ms \n", u32EndOPTime - u32StartOPTime);
				...
				if(s_bStartRecToFile && s_pAFAECPathFile == NULL){
					//Create before AEC Path file
					s_pAFAECPathFile = fopen ( "/mnt/mmc/AEC3_After_AEC_path", "w");

					if(s_pAFAECPathFile == NULL){
						printf("Unable create after AEC path file \n");
						s_bStartRecToFile = false;
					}
				}
				
				if(s_pAFAECPathFile){
					fwrite(s_pu8RecPCMBuf, 1, s_u32RecFrameSize, s_pAFAECPathFile);
				}

				if(s_bStartRecToFile == false && s_pAFAECPathFile){
					//close play Path file
					fclose(s_pB4AECPathFile);			
					s_pAFAECPathFile = NULL;
				}
...
//				memcpy((s_pu8RecPCMBuf), pu8TempRecBuf, s_sAudioRecCtx.u32DataSize);
		...
		if(i32WriteByte < 0){
			...
			DEBUGPRINT("Client had issued a close \n");
			break;
		}
		else{
...
)
{
	...
	while(s_bThreadRun){
		...
		if(i32ReadByte < 0){
			...
			DEBUGPRINT("Receive PCM data from client %d bytes \n", i32ReadByte);
		}
...
int main(int argc, char **argv)
{
	...
	i32ServSD = socket(AF_INET, SOCK_STREAM, 0);
	if(i32ServSD < 0){
		perror("Server-socket() error");
		exit(-1);
	}
	else{
		DEBUGPRINT("Server-socket() OK \n");
	}
	...
	i32Err = setsockopt(i32ServSD, SOL_SOCKET, SO_REUSEADDR, &i32On, sizeof(i32On));
	if(i32Err < 0){
		...
		DEBUGPRINT("Server-setsockopt() OK \n");
	}
	...
	DEBUGPRINT("Using %s, listening at %d\n", inet_ntoa(sServerAddr.sin_addr), SERVPORT);
	...
	i32Err = bind(i32ServSD, (struct sockaddr *)&sServerAddr, sizeof(sServerAddr));
	if(i32Err < 0){
	...
	/*************************************************/
	/* Up to 1 clients can be queued */
	i32Err = listen(i32ServSD, 1);
	if(i32Err < 0){
		...
		DEBUGPRINT("Server-Reasy for client connection.. \n");
	}
	...
	uint32_t u32SinSize = sizeof(struct sockaddr_in);
	i32ClientSD = accept(i32ServSD, (struct sockaddr *)&sClientAddr, &u32SinSize);
	if(i32ClientSD < 0){
		...
		DEBUGPRINT("Server-accept() OK \n");
	}

	/*client ip */
	DEBUGPRINT("Server-new socket i32ClientSD is OK...\n");
	DEBUGPRINT("Got connection from the client: %s \n", inet_ntoa(sClientAddr.sin_addr));
