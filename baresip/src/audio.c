...
static int start_source(struct autx *tx, struct audio *a, struct list *ausrcl)
{
	...
	if (!tx->ausrc && ausrc_find(ausrcl, NULL) && !a->hold) {
		...
		printf("DDDDDDDDDD file: %s, line %d, tx->psize %d \n", __FILE__, __LINE__, tx->psize);
		      ...
		      int pt_tx, const char *params)
{
	...
	if (ac->ptime) {
		...
		//CHChen59. remove tx->psize code  
//		tx->psize = sz * calc_nsamp(ac->srate, ac->ch, ac->ptime);
		///////////////////////////////////////////////////////
		printf("DDDDDDDDDD file: %s, line %d, tx->psize %d, ac->srate %d, ac->ch %d, ac->ptime %d \n", __FILE__, __LINE__, tx->psize, ac->srate, ac->ch, ac->ptime);
	}
