...
static void *write_thread(void *arg)
{
	...
	while (st->run) {
		...
		// modify by CHChen59 05/25/2021
		snd_pcm_sframes_t avail_frames;
		avail_frames = snd_pcm_avail(st->write);
		
		if(avail_frames == -EPIPE)
			snd_pcm_prepare(st->write);

		if(avail_frames < num_frames) {
			usleep(1000);
			continue;
		}
		//////////////////////////
		...
		if (snd_pcm_state(st->write) == SND_PCM_STATE_PREPARED) {
			snd_pcm_start(st->write);
		}
